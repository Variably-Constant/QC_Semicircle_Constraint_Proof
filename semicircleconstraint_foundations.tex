...
\begin{enumerate}
    ...
    \item \textbf{Constant Fisher information}: The information-theoretic distance along the semicircle is uniform, with total arc length equal to $\pi$.
\end{enumerate}

We further demonstrate that this constraint has practical applications to variational quantum algorithms (VQAs)~\cite{Peruzzo2014,Farhi2014}, where it explains the geometric origin of barren plateaus~\cite{McClean2018,Cerezo2021} and provides design principles for optimization.
...
The constraint~\eqref{eq:semicircle} has clear geometric meaning:
\begin{itemize}
...
This induces a metric on probability space with line element:
\begin{equation}
ds^2 = \frac{dq^2}{4q(1-q)} \implies ds = \frac{dq}{2\sqrt{q(1-q)}}
\end{equation}

\begin{theorem}[Arc Length = $\pi$]
The Fisher information distance along the semicircle from $q = 0$ to $q = 1$ equals $\pi$:
\begin{equation}
\boxed{L = \int_0^1 \frac{dq}{2\sqrt{q(1-q)}} = \pi}
\end{equation}
\end{theorem}
...
\end{equation}
As $q: 0 \to 1$, we have $\theta: 0 \to \pi/2$. The arc length is:
\begin{equation}
L = 2\int_0^{\pi/2} d\theta = \pi
\end{equation}
Alternatively, via the Beta function: $\int_0^1 q^{-1/2}(1-q)^{-1/2}\,dq = B(1/2, 1/2) = \Gamma(1/2)^2 = \pi$.
\end{proof}

This result confirms the semicircle has information-geometric arc length $\pi$, connecting to quantum information geometry~\cite{Nielsen2010}. The constant Fisher information ($I_F = 1$ in angular coordinates) reflects uniform ``information density'' along the trajectory.
...
\begin{itemize}
    ...
    \item Our work provides a unified geometric perspective on these phenomena via the semicircle constraint.
\end{itemize}
...
We have presented the semicircle constraint $(q - \frac{1}{2})^2 + C_{qc}^2 = \frac{1}{4}$ as a geometric framework for understanding quantum-classical correlation. Key results:
...
\begin{enumerate}
    ...
    \item The Fisher information is constant along the semicircle trajectory, with arc length equal to $\pi$.
